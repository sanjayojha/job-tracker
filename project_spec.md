...
| AI tooling            | Laravel Boost (MCP server)                                                                 | Live app introspection: schema, config, logs, version-correct docs. Runs via `ddev artisan boost:mcp` |
...
7. **Styling → Tailwind CSS 4, explicitly not v3.** v4 drops the JS config file and PostCSS step in favour of a Vite plugin and CSS-native `@theme`. Worth noting because most Tailwind material still documents v3, and its `@tailwind` directives and `content` globs do not apply here.
