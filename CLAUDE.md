...
GitHub Actions runs the same lint and test steps on every push to `main` and every PR (`.github/workflows/ci.yml`), **except when a change touches only prose** — `**.md`, `docs/**`, and `.claude/**` are in `paths-ignore`, so a docs-only commit triggers no run at all and none is expected. A commit touching both prose and code still runs. **Pushing a feature branch runs nothing** — the workflow has no `push` trigger for branches other than `main`, so CI only sees the branch once a PR is open. Open the PR when you push, not when the work feels finished.
