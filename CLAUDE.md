...
- **`routes/api.php` does not exist yet.** Laravel 13 needs `ddev artisan install:api` before API routes work.
...
**Component library is undecided.** Nothing is installed. Build with plain Tailwind until a real need appears; if you think one is warranted, raise it rather than adding it.

**Open UI decisions** — routing, state management, and API client are all unchosen. Decide when the first real screen lands, don't pre-commit. Ask rather than assume.
